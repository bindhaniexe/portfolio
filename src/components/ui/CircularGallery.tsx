...
function createTextTexture(gl: OGLRenderingContext, text: string, font = 'bold 30px monospace', color = 'white') {
...
class Media {
  ...
  constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius = 0, font }: MediaOptions) {
  ...
  createShader() {
    // Card size for rasterising logos — square, large enough for crisp rendering
    const CARD_SIZE = 512;
    const LOGO_SIZE = 300;
    ...
    this.program = new Program(this.gl, {
      ...
      uniforms: {
        ...
        uImageSizes: { value: [CARD_SIZE, CARD_SIZE] },
        uSpeed: { value: 0 },
    ...
    const buildCard = (img?: HTMLImageElement) => {
      const canvas = document.createElement('canvas');
      canvas.width = CARD_SIZE;
      canvas.height = CARD_SIZE;
      const ctx = canvas.getContext('2d')!;
      const r = CARD_SIZE * 0.12;
      const w = CARD_SIZE, h = CARD_SIZE;
      // Dark rounded-rect card background
      ...
      ctx.fill();
      if (img) {
        const offset = (CARD_SIZE - LOGO_SIZE) / 2;
        ctx.drawImage(img, offset, offset, LOGO_SIZE, LOGO_SIZE);
      }
      texture.image = canvas;
    ...
    img.onload = () => buildCard(img);
    img.onerror = () => buildCard();
    img.src = this.image;
  }
  ...
  onResize({ screen, viewport }: { screen?: { width: number; height: number }; viewport?: { width: number; height: number } } = {}) {
    ...
    this.program.uniforms.uPlaneSizes.value = [(this.plane.scale as { x: number }).x, (this.plane.scale as { y: number }).y];
    this.padding = 2;
...
class App {
  ...
  constructor(container: HTMLElement, { items, bend = 1, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05 }: AppOptions = {}) {
  ...
  destroy() {
    ...
    if (this.renderer?.gl?.canvas && (this.renderer.gl.canvas as HTMLCanvasElement).parentNode) {
